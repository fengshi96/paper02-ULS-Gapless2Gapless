...
def main(total, cmdargs):
	...
	print(popt*6)
        ...
        #print fittings
	fit = np.array(( [x[int(start/N*400):int(end/N*400)], func(x[int(start/N*400):int(end/N*400)], popt[0], popt[1]) ])).T
	...
	ax.plot(x[int(start/N*400):int(end/N*400)], func(x[int(start/N*400):int(end/N*400)], popt[0], popt[1]), '-', color='teal')
...
def func(x, a, b):
    return a * np.log(2 * N * np.sin(pi * x / N) / pi ) + b#+ b * (np.cos(2 * pi * x / 3))/ (np.abs( N   * np.sin(pi * x / N)) ** (2.0/3.0)) + c
